...
namespace AS {
	...
	{
		auto StepLength =  Calc_dist(sp, tp)> Calc_dist(ep, tp) ? Calc_dist(ep, tp)/ DELTA : Calc_dist(sp, tp)/ DELTA;
		
		
		A_STAR MyAstar(sp, tp, ep, radav, mg, e_w1, survice_w1, start_w1, end_w1, horizontal_corner1, verticality_corner1, hmin1, hmax1, StepLength);

		int MAX_It = int(Calc_dist(sp, ep) / StepLength) * 20;
